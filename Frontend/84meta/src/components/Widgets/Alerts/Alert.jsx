...
const Alert = ({type, message, duration, open}) => {
    ...
    const alertType = {
        ...
        "auth":{
            "title":"I'm a Authentication Alert",
            "message":"Explaining the alert and what to do about it",
            "icon":<svg className="w-7 h-7 md:w-9 md:h-9" width="42" height="42" viewBox="0 0 42 42" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path d="M41 21C41 9.9543 32.0457 1 21 1C9.9543 1 1 9.9543 1 21C1 32.0457 9.9543 41 21 41C32.0457 41 41 32.0457 41 21Z" fill="#3BB75E"/>
            <path d="M10.1851 21.113L17.1436 27.8174L31.0604 14.4087" fill="#3BB75E"/>
  ...
  return (
    ...
    //     </div>
    // </div>
    <div className={`${alertType[type] ? alertType[`${type ? type : 'success'}`]["bgColor"] : 'bg-brandYellow3x'} text-white font-medium ${alertOpen ? '' : '-translate-y-[300%]'} transition-transform ease-out duration-500 items-center w-fit flex gap-8 text-sm lg:text-base py-2 pl-4 pr-2 justify-between rounded-10 drop-shadow-lg fixed top-8 left-[50%] -translate-x-[50%]`}>
        <div className={`text-left capitalize`}>
            {alertType[type] ? message ? message : alertType[`${type ? type : 'success'}`]["message"] : 'Invalid Alert Type! Types are: success, warning and danger'}
        </div>
        <button onClick={closeAlert} className='font-jost text-sm lg:text-base py-1 px-4 hover:bg-red-500 hover:text-white text-white rounded-10'>x</button>
    </div>
  )
}
